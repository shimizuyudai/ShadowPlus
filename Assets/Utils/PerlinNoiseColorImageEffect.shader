Shader "Hidden/PerlinNoiseColor"
{
	Properties
	{
	...
	SubShader
	{
		...
		float fBm(fixed2 st)
		{
			...
			{
				float hue = fBm(float2(i.uv.x*_Rate.x + _ElapsedTime + _Offset.x, i.uv.y*_Rate.y + _ElapsedTime + _Offset.y));
				float4 col = float4(HSVtoRGB(float3(hue,_Saturation,_Brightness)), 1);
				return col;
			}
			ENDCG
		}
